...
var CreditCardGateway, CustomerGateway, Digest, Environment, TransactionGateway, TransparentRedirectGateway, Util, dateFormat, exceptions, querystring;
...
TransparentRedirectGateway = (function() {
  ...
  function TransparentRedirectGateway(gateway) {
    var fullHost, uriScheme;
    this.gateway = gateway;
    uriScheme = this.gateway.config.environment.ssl ? 'https://' : 'http://';
    fullHost = this.gateway.config.environment === Environment.Development ? "" + this.gateway.config.environment.server + ":" + this.gateway.config.environment.port : this.gateway.config.environment.server;
    this.url = "" + uriScheme + fullHost + this.gateway.config.baseMerchantPath + "/transparent_redirect_requests";
  }

  TransparentRedirectGateway.prototype.generateTrData = function(inputData) {
    var data, dataSegment, trDataHash;
    ...
    trDataHash = Digest.hexdigest(this.gateway.config.privateKey, dataSegment);
  ...
  TransparentRedirectGateway.prototype.validateQueryString = function(queryString) {
    ...
    return Digest.hexdigest(this.gateway.config.privateKey, matches[1]) === matches[2];
  };
  ...
  TransparentRedirectGateway.prototype.confirm = function(queryString, callback) {
    ...
    return this.gateway.http.post('/transparent_redirect_requests/' + params.id + '/confirm', null, confirmCallback);
  };
